...
@Serializable
data class TogetherOnlineIssueTokenRequest(
    val scopes: List<String> = emptyList(),
)

@Serializable
data class TogetherOnlineIssueTokenResponse(
    val token: String,
...
class TogetherOnlineApi(
    ...
    suspend fun issueRuntimeToken(
        scopes: List<String> = emptyList(),
    ): TogetherOnlineIssueTokenResponse =
        withRetry {
            val payload =
                json.encodeToString(
                    TogetherOnlineIssueTokenRequest.serializer(),
                    TogetherOnlineIssueTokenRequest(scopes = scopes),
                )
            val resp =
                client.post("$v1BaseUrl/auth/token") {
                    contentType(ContentType.Application.Json)
                    setBody(payload)
                }
            val status = resp.status.value
            val raw = resp.bodyAsText()
            if (status !in 200..299) throw TogetherOnlineApiException(errorMessageFromResponse(status, raw), statusCode = status)
            json.decodeFromString(TogetherOnlineIssueTokenResponse.serializer(), raw)
        }
}
