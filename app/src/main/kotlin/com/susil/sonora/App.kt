...
import com.susil.sonora.together.TogetherOnlineEndpoint
...
class App : Application(), SingletonImageLoader.Factory {
    ...
    private fun initializeCriticalSync() {
        ...
        applicationScope.launch(Dispatchers.IO) {
            val baseUrl = TogetherOnlineEndpoint.baseUrlOrNull(dataStore)
            if (!baseUrl.isNullOrBlank()) {
                val token =
                    TogetherRuntimeAuth.tokenOrNull(
                        dataStore = dataStore,
                        baseUrl = baseUrl,
                        fallbackStaticToken = BuildConfig.TOGETHER_BEARER_TOKEN,
                    )
                val canvasBase = TogetherRuntimeAuth.canvasProxyBaseUrl(baseUrl)
                SonoraCanvas.initialize(
                    bearerToken = token ?: BuildConfig.CANVAS_BEARER_TOKEN,
                    baseUrl = canvasBase,
                )
            }
        }
